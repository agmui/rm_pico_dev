...
{
    drivers->motorHandler.addMotorToManager(this);
    // attachSelfToRxHandler(); //TODO
}

void DjiMotor::processMessage(const can2040_msg& message)
{
    if (message.id != DjiMotor::getMotorIdentifier())
    {
        return;
    }
...
void DjiMotor::setDesiredOutput(int32_t desiredOutput)
{
    int16_t desOutputNotInverted =
        static_cast<int16_t>(pico::algorithms::limitVal<int32_t>(desiredOutput, SHRT_MIN, SHRT_MAX));
...
pico::can::PioNum DjiMotor::getCanBus() const { return motorCanBus; }
